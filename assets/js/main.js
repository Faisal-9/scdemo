...
document.querySelectorAll(".nav-item-top .has-dropdown").forEach((link) => {
  link.addEventListener("click", function (e) {
    if (window.innerWidth <= 768) {
      ...
      document.querySelectorAll(".nav-item-top .dropdown-menu-top").forEach((menu) => {
        if (menu !== dropdown) menu.classList.remove("active");
      });
      if (dropdown) dropdown.classList.toggle("active");
    }
...
if (typeof Swiper !== "undefined" && document.querySelector(".heroSwiper")) {
  ...
  if (indicatorsContainer) {
    indicatorsContainer.addEventListener("mouseenter", () => {
      ...
      indicators.forEach((ind) => {
        ind.querySelector(".hero-indicator-progress").style.animationPlayState = "paused";
      });
    });
    ...
    indicatorsContainer.addEventListener("mouseleave", () => {
      ...
      indicators.forEach((ind) => {
        ind.querySelector(".hero-indicator-progress").style.animationPlayState = "running";
      });
    });
  }
...
(function () {
  ...
  function setPosition(x) { track.style.transform = `translateX(${x}px)`; }
...
if (header) {
  ...
  window.addEventListener("scroll", () => {
    ...
    if (!isSmall && scrollY > 100) { header.classList.add("header-small");    isSmall = true;  }
    if (isSmall  && scrollY < 60)  { header.classList.remove("header-small"); isSmall = false; }
  });
}
...
const aboutLinks  = document.querySelectorAll(".about-sidebar .nav-link[data-section]");
...
(function () {
  ...
  function show(item) {
    ...
    setTimeout(() => {
      ...
      if (imgEl)  { imgEl.src = item.dataset.img; imgEl.alt = item.dataset.title; }
  ...
  function stopTimeline() { clearInterval(interval); }
  ...
  /* OBSERVER — threshold 0 so it fires on any visibility */
  if (section && "IntersectionObserver" in window) {
    ...
    playTimeline(); // fallback: just start immediately
  }
...
document.querySelectorAll(".sub-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    document.querySelectorAll(".subservice-content").forEach((c) => c.classList.remove("active"));
...
document.querySelectorAll(".sub-sub-list li[data-target]").forEach((item) => {
  item.addEventListener("click", () => {
    ...
    container.querySelectorAll("li[data-target]").forEach((li) => li.classList.remove("active"));
  ...
  if (serviceTab) {
    ...
    if (hashAnchor) {
      setTimeout(() => {
        ...
        if (targetSubservice && targetSubservice.classList.contains("subservice-content")) {
          activeSection.querySelectorAll(".subservice-content").forEach((sc) => sc.classList.remove("active"));
          targetSubservice.classList.add("active");

          const correspondingSubTab = activeSection.querySelector(`.sub-tab[data-sub="${targetSubservice.id}"]`);
          if (correspondingSubTab) {
            activeSection.querySelectorAll(".sub-tab").forEach((tab) => tab.classList.remove("active"));
...
if (sectorMenuItems.length) {
  ...
  function updateActiveSectorData() {
    ...
    activeSectorBlocks = Array.from(activeSector.querySelectorAll(".about-section-block"));
    activeSectorLinks  = Array.from(activeSector.querySelectorAll(".about-sidebar .nav-link"));
  }
  ...
  function highlightSectorSidebar() {
    ...
    activeSectorBlocks.forEach((section, index) => {
      ...
      if (scrollPos >= top && scrollPos < bottom) {
        ...
        if (activeSectorLinks[index]) activeSectorLinks[index].classList.add("active");
      }
    });
  }
  ...
  function activateSectorTab(target) {
    ...
    const matchedMenu = Array.from(sectorMenuItems).find((el) => el.dataset.target === target);
  ...
  sectorMenuItems.forEach((item) => {
    item.addEventListener("click", function () {
      ...
      if (page) window.scrollTo({ top: page.offsetTop - 90, behavior: "smooth" });
    });
  });
...
function attachCategoryListeners() {
  document.querySelectorAll(".project-category-filters button").forEach((btn) => {
    btn.addEventListener("click", function () {
      document.querySelectorAll(".project-category-filters button").forEach((b) => b.classList.remove("active"));
      this.classList.add("active");
...
function filterProjects() {
  ...
  document.querySelectorAll(".project-category-section").forEach((section) => {
    const visibleItems = section.querySelectorAll(".project-item[style*='flex']");
...
if (typeof urlSector !== "undefined" && typeof urlCategory !== "undefined") {
  if (urlCategory !== "all") {
    ...
    updateCategoryButtons("all");
    document.querySelectorAll(".project-category-filters button").forEach((b) => {
  ...
  document.querySelectorAll(".media-split").forEach((split) => {
    ...
    items.forEach((item) => {
      item.addEventListener("click", function () {
        ...
        history.replaceState(null, "", window.location.pathname + "?tab=" + this.dataset.tab + "&id=" + this.dataset.id);
      });
    });
  });
})();
...
document.querySelectorAll(".sub-sub-list").forEach((menuList) => {
  menuList.querySelectorAll(".group-title").forEach((t) => t.classList.remove("active"));
  menuList.querySelectorAll(".group-list").forEach((l) => l.classList.remove("active"));
});
...
document.querySelectorAll(".sub-sub-list .group-title").forEach((title) => {
  title.addEventListener("click", function () {
    ...
    parentList.querySelectorAll(".group-list").forEach((l) => l.classList.remove("active"));
    parentList.querySelectorAll(".group-title").forEach((t) => t.classList.remove("active"));
    parentList.querySelectorAll("li[data-target]").forEach((i) => i.classList.remove("active"));
    ...
    if (firstChildItem) {
      ...
      if (activeContent) {
        activeContent.querySelectorAll(".detail-panel").forEach((p) => p.classList.remove("active"));
        const targetPanel = document.getElementById(firstChildItem.dataset.target);
        if (targetPanel) targetPanel.classList.add("active");
      }
    }
  });
