...
$slide = $id ? HomeManager::slide((int)$id) : null;
if ($id && !$slide) redirect(adminUrl('homepage/'));
$error = null;
if (isPost()) {
    CSRF::verify($_POST['csrf_token'] ?? null);
    try {
        $new = HomeManager::saveSlide($_POST, $id ?: null);
        Auth::audit(Auth::id(), $id ? 'update' : 'create', 'home_hero_slide', $new, ($id ? 'Updated' : 'Created') . ' homepage hero slide: ' . ($_POST['title'] ?? ''));
        flash('success', 'Hero slide saved.');
        redirect(adminUrl('homepage/slide.php?id=' . $new));
    } catch (Throwable $e) {
        $error = APP_DEBUG ? $e->getMessage() : 'The hero slide could not be saved.';
        $slide = array_merge((array)$slide, $_POST);
    }
}
$pageTitle = $slide ? 'Edit Hero Slide' : 'Add Hero Slide';
$activeNav = 'homepage';
require __DIR__ . '/../partials/header.php';
require __DIR__ . '/../partials/sidebar.php';
?>
<main class="admin-content"><?php $breadcrumbs = [['label' => 'Dashboard', 'url' => adminUrl('dashboard.php')], ['label' => 'Homepage', 'url' => adminUrl('homepage/')], ['label' => $slide ? 'Edit Hero Slide' : 'Add Hero Slide', 'url' => null]];
                            require __DIR__ . '/../partials/breadcrumbs.php';
                            $heading = $slide ? 'Edit Hero Slide' : 'Add Hero Slide';
                            $description = 'Preserve the existing legacy ID for migrated slides.';
                            $actionUrl = adminUrl('homepage/');
                            $actionLabel = 'Back';
                            require __DIR__ . '/../partials/page-heading.php';
                            require __DIR__ . '/../partials/alerts.php'; ?>
    <?php if ($error): ?><div class="alert alert-error"><?= e($error) ?></div><?php endif; ?><section class="form-card">
        <form method="post"><?= CSRF::field() ?><div class="form-grid">
                <div class="form-field"><label>Legacy ID</label><input name="legacy_id" value="<?= e($slide['legacy_id'] ?? '') ?>" required></div>
