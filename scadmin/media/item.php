...
if ($id && !$row) {
    Session::flash('error', 'Media item not found.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $errors[] = $e->getMessage();
        $row = array_merge((array)$row, $_POST, ['descriptions' => $data['descriptions'] ?? []]);
    }
}
if (!$row) {
...
<main class="admin-content">
    <?php $breadcrumbs = [['label' => 'Dashboard', 'url' => adminUrl('dashboard.php')], ['label' => 'Media', 'url' => adminUrl('media/')], ['label' => $pageTitle, 'url' => null]];
    require __DIR__ . '/../partials/breadcrumbs.php';
    $heading = $pageTitle;
    $description = 'All fields map directly to the existing public media data shape.';
    $actionUrl = null;
    $actionLabel = null;
    ...
    <section class="content-panel">
        <form method="post" class="admin-form"><input type="hidden" name="csrf_token" value="<?= e(CSRF::token()) ?>">
            <div class="form-grid">
                <label>Type<select name="media_type" required>
                        <option value="news" <?= $row['media_type'] === 'news' ? 'selected' : '' ?>>News</option>
                        <option value="events" <?= $row['media_type'] === 'events' ? 'selected' : '' ?>>Events</option>
                        <option value="gallery" <?= $row['media_type'] === 'gallery' ? 'selected' : '' ?>>Gallery</option>
                    </select></label>
...
<script>
    document.getElementById('addDescription').addEventListener('click', function() {
        const w = document.getElementById('descriptionRows');
        const d = document.createElement('div');
        d.className = 'repeat-row';
        d.innerHTML = '<textarea name="descriptions[]" rows="4"></textarea><button type="button" class="small-button js-remove-row">Remove</button>';
        w.appendChild(d);
    });
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('js-remove-row')) {
            const rows = document.querySelectorAll('#descriptionRows .repeat-row');
