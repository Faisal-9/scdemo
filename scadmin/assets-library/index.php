...
?><main class="admin-main management-page">
    <div class="admin-container">
        <div class="media-library-hero management-hero">
            ...
            <div class="media-library-actions"><a class="btn btn-primary" href="<?= e(adminUrl('assets-library/upload.php')) ?>">Upload</a><a class="btn btn-secondary" href="<?= e(adminUrl('assets-library/picker.php')) ?>">Open Picker</a></div>
        </div>
        ...
        <div class="media-library-grid"><?php foreach ($rows as $row): $image = str_starts_with((string)$row['mime_type'], 'image/'); ?><article class="media-library-card">
                    <div class="media-library-thumb"><?php if ($image): ?><img src="<?= e(function_exists('baseUrl') ? baseUrl($row['relative_path']) : $row['relative_path']) ?>" alt="<?= e((string)($row['alt_text'] ?? '')) ?>" loading="lazy"><?php else: ?><span><?= e(strtoupper(pathinfo((string)$row['original_name'], PATHINFO_EXTENSION) ?: 'FILE')) ?></span><?php endif; ?></div>
