...
$existing = $editing && $id ? ($type === 'values' ? AboutManager::coreValue($id) : AboutManager::item($type, $id)) : null;
...
<main class="admin-content">
    ...
    $heading = $editing ? 'Edit ' . $title : 'Manage ' . $title;
    $description = $type === 'timeline' ? 'Keep the current six milestones and ordering during initial migration.' : 'Existing records are editable without changing the public markup.';
    ...
    <?php if ($editing): ?>
        <section class="form-card">
            <form method="post"><?= CSRF::field() ?><div class="form-grid">
                    <?php if ($type === 'timeline'): ?><div class="form-field"><label>Year</label><input name="year" value="<?= e($existing['year'] ?? '') ?>" required></div>
    ...
    <?php else: ?>
        <section class="content-panel">
            <div class="panel-heading">
                <div>
                    <h2><?= e($title) ?></h2>
                    <p class="muted">Use existing asset paths. File upload management is intentionally deferred.</p>
                </div><a class="button-link" href="<?= e(adminUrl('about/items.php?type=' . $type . '&edit=1')) ?>">+ Add record</a>
            </div>
            <div class="table-wrap">
                <table class="admin-table">
                    <thead>
                        <tr>
                            <th>Order</th>
                            <th><?= e($type === 'values' ? 'Value' : 'Name') ?></th>
                            <th>Asset</th>
                            <th>Status</th>
                            <th></th>
                        </tr>
                    ...
                    <tbody>
                        <?php foreach ($rows as $row): $editUrl = adminUrl('about/items.php?type=' . $type . '&edit=1&id=' . (int)$row['id']); ?><tr>
                                <td><?= e((string)$row['sort_order']) ?></td>
                                <td><?= e($type === 'values' ? $row['value_text'] : ($row['name'] ?? '')) ?></td>
                                <td><?= e($type === 'values' ? '—' : $row['logo_path'] ?? ($row['image_path'] ?? '')) ?></td>
                                <td><span class="status-badge status-<?= $row['is_active'] ? 'active' : 'inactive' ?>"><?= $row['is_active'] ? 'Active' : 'Inactive' ?></span></td>
                                <td><a class="small-button" href="<?= e($editUrl) ?>">Edit</a>
                                    <form method="post" style="display:inline" onsubmit="return confirm('Delete this record?');"><?= CSRF::field() ?><input type="hidden" name="delete_item_id" value="<?= e((string)$row['id']) ?>"><button class="small-button small-button-danger" type="submit">Delete</button></form>
                                </td>
                            </tr><?php endforeach; ?>
                    </tbody>
                </table>
