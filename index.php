<!DOCTYPE html>
<?php
$page_title = "State Corps - Engineering Afghanistan's Infrastructure Future";
$page_description = "Leading infrastructure company delivering 100+ projects valued at $600M+ for government and international partners since 2007";
include("includes/head.php");
?>

<body>
    <!-- ================= HEADER ================= -->
    <?php include("includes/header.php"); ?>

    <!-- ================= HERO SLIDER ================= -->
    <?php include("includes/slider.php"); ?>

    <!-- ================= State Corps In Numbers ================= -->
    <?php include("includes/indexScInNumbers.php"); ?>

    <!-- ================= OUR SERVICES ================= -->
    <?php include("includes/indexServices.php"); ?>

    <!-- ================= ABOUT US ================= -->
    <?php include("includes/indexAboutUs.php"); ?>

    <!-- ================= LATEST NEWS ================= -->
    <?php include("includes/indexNews.php"); ?>

    <!-- ================= CLIENTS ================= -->
    <?php include("includes/indexClients.php"); ?>

    <!-- ================= FOOTER ================= -->
    <?php include("includes/footer.php"); ?>

    <!-- ================= LINKS OF FOOTER ================= -->
    <?php include("includes/footerLink.php"); ?>


</body>

</html>