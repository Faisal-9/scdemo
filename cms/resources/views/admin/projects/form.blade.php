...
<form method="post" enctype="multipart/form-data" action="{{ $project->exists ? route('admin.projects.update', $project) : route('admin.projects.store') }}">
    ...
    @endforeach
    <label>Sector<select name="sector_id">
            <option value="">None</option>@foreach($sectors as $sector)<option value="{{ $sector->id }}" @selected(old('sector_id', $project->sector_id) == $sector->id)>{{ $sector->title }}</option>@endforeach
        </select></label><br>
