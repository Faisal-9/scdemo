...
final class ContactMessageManager
{
    ...
    public static function paginate(int $page = 1, int $perPage = 20, string $search = '', string $status = ''): array
    {
        ...
        $db = Database::connection();
        $st = $db->prepare('SELECT COUNT(*) FROM contact_messages' . $ws);
        $st->execute($params);
        $total = (int)$st->fetchColumn();
        $pages = max(1, (int)ceil($total / $perPage));
        $page = min($page, $pages);
        $st = $db->prepare('SELECT * FROM contact_messages' . $ws . ' ORDER BY created_at DESC,id DESC LIMIT :lim OFFSET :off');
        foreach ($params as $k => $v) $st->bindValue(':' . $k, $v);
        $st->bindValue(':lim', $perPage, PDO::PARAM_INT);
        $st->bindValue(':off', ($page - 1) * $perPage, PDO::PARAM_INT);
        $st->execute();
        return ['items' => $st->fetchAll(PDO::FETCH_ASSOC), 'total' => $total, 'page' => $page, 'pages' => $pages];
    }
    public static function find(int $id): ?array
    {
        $st = Database::connection()->prepare('SELECT * FROM contact_messages WHERE id=:id LIMIT 1');
        $st->execute(['id' => $id]);
    ...
    {
        if (!in_array($status, ['unread', 'read', 'archived'], true)) throw new InvalidArgumentException('Invalid status.');
        $st = Database::connection()->prepare('UPDATE contact_messages SET status=:status WHERE id=:id');
        $st->execute(['status' => $status, 'id' => $id]);
    }

    public static function page(): array
    {
        return self::single('contact_page');
    }
    public static function qrCodes(): array
    {
        return Database::connection()->query('SELECT * FROM contact_qr_codes ORDER BY sort_order ASC,id ASC')->fetchAll(PDO::FETCH_ASSOC);
    }
    public static function offices(): array
    {
        return Database::connection()->query('SELECT * FROM contact_offices ORDER BY sort_order ASC,id ASC')->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public static function savePage(array $d): void
    {
        ...
        $st->execute(['section_title' => $d['section_title'], 'head_office_title' => $d['head_office_title'], 'head_office_phone' => $d['head_office_phone'], 'head_office_whatsapp' => $d['head_office_whatsapp'] ?? null, 'head_office_email' => $d['head_office_email'], 'head_office_address' => $d['head_office_address'], 'map_label' => $d['map_label'], 'map_lat' => $d['map_lat'], 'map_lng' => $d['map_lng'], 'map_zoom' => max(1, min(20, (int)($d['map_zoom'] ?? 14))), 'form_title' => $d['form_title'], 'overseas_title' => $d['overseas_title'], 'overseas_subtitle' => $d['overseas_subtitle']]);
    }
    public static function saveQr(array $d, ?int $id = null): int
    {
        $assetId = AssetResolver::id($d['asset_id'] ?? null);
        $label = trim((string)($d['label'] ?? ''));
        if (!$assetId || $label === '') throw new InvalidArgumentException('QR image and label are required.');
        $p = ['asset_id' => $assetId, 'label' => $label, 'sort_order' => max(0, (int)($d['sort_order'] ?? 0)), 'is_active' => !empty($d['is_active']) ? 1 : 0];
        $db = Database::connection();
        if ($id === null) {
            $st = $db->prepare('INSERT INTO contact_qr_codes (asset_id,label,sort_order,is_active) VALUES (:asset_id,:label,:sort_order,:is_active)');
            $st->execute($p);
            return (int)$db->lastInsertId();
        }
        $p['id'] = $id;
        $st = $db->prepare('UPDATE contact_qr_codes SET asset_id=:asset_id,label=:label,sort_order=:sort_order,is_active=:is_active WHERE id=:id');
        $st->execute($p);
        return $id;
    }
    public static function saveOffice(array $d, ?int $id = null): int
    {
        $title = trim((string)($d['title'] ?? ''));
        $address = trim((string)($d['address'] ?? ''));
        if ($title === '' || $address === '') throw new InvalidArgumentException('Office title and address are required.');
        $p = ['title' => $title, 'phone' => trim((string)($d['phone'] ?? '')) ?: null, 'whatsapp' => trim((string)($d['whatsapp'] ?? '')) ?: null, 'email' => trim((string)($d['email'] ?? '')) ?: null, 'address' => $address, 'sort_order' => max(0, (int)($d['sort_order'] ?? 0)), 'is_active' => !empty($d['is_active']) ? 1 : 0];
        $db = Database::connection();
        if ($id === null) {
            $st = $db->prepare('INSERT INTO contact_offices (title,phone,whatsapp,email,address,sort_order,is_active) VALUES (:title,:phone,:whatsapp,:email,:address,:sort_order,:is_active)');
            $st->execute($p);
            return (int)$db->lastInsertId();
        }
        $p['id'] = $id;
        $st = $db->prepare('UPDATE contact_offices SET title=:title,phone=:phone,whatsapp=:whatsapp,email=:email,address=:address,sort_order=:sort_order,is_active=:is_active WHERE id=:id');
        $st->execute($p);
        return $id;
    }
    public static function delete(string $type, int $id): void
    {
        $table = ['qr' => 'contact_qr_codes', 'office' => 'contact_offices'][$type] ?? null;
        if (!$table) throw new InvalidArgumentException('Invalid contact record.');
        $st = Database::connection()->prepare("DELETE FROM {$table} WHERE id=:id");
        $st->execute(['id' => $id]);
    }
    private static function single(string $table): array
    {
        $r = Database::connection()->query("SELECT * FROM {$table} WHERE id=1 LIMIT 1")->fetch(PDO::FETCH_ASSOC);
        return $r ?: [];
    }
}
