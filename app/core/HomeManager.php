...
final class HomeManager
{
    ...
    {
        $rows = Database::connection()->query('SELECT * FROM home_hero_slides ORDER BY sort_order ASC, id ASC')->fetchAll();
        return array_map(static fn(array $row): array => AssetResolver::hydrate($row, ['hero_asset_id' => 'image_path']), $rows);
    }
    ...
    public static function slide(int $id): ?array
    {
        ...
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
        return is_array($row) ? AssetResolver::hydrate($row, ['hero_asset_id' => 'image_path']) : null;
    }
    ...
    public static function saveSlide(array $data, ?int $id = null): int
    {
        ...
        $image = AssetResolver::id($data['hero_asset_id'] ?? null);
        if ($title === '') throw new RuntimeException('Hero slide title is required.');
        if (!$image) throw new RuntimeException('Hero slide image is required.');

        if ($id === null) {
            $stmt = $pdo->prepare(
                'INSERT INTO home_hero_slides
                    (legacy_id, title, description, hero_asset_id, indicator, sort_order, is_active)
                      VALUES
                          (:legacy_id, :title, :description, :hero_asset_id, :indicator, :sort_order, :is_active)'
            );
            $stmt->execute([
    ...
    public static function saveHistory(array $data, ?int $id = null): int
    {
        ...
        if ($id === null) {
            $stmt = $pdo->prepare('INSERT INTO home_history (year,title,sort_order,is_active) VALUES (:year,:title,:sort_order,:is_active)');
            $stmt->execute($params);
            return (int)$pdo->lastInsertId();
        }
        $params[':id'] = $id;
        $stmt = $pdo->prepare('UPDATE home_history SET year=:year,title=:title,sort_order=:sort_order,is_active=:is_active WHERE id=:id');
        $stmt->execute($params);
        return $id;
    }
    ...
    public static function whyTabs(): array
    {
        ...
        foreach ($tabs as &$tab) {
            $tab = AssetResolver::hydrate($tab, ['why_asset_id' => 'image_path']);
    ...
    public static function saveWhyTab(array $data, ?int $id = null): int
    {
        ...
        try {
            $params = [':legacy_id' => self::nullable($data['legacy_id'] ?? null), ':tab_name' => $tabName, ':title' => self::nullable($data['title'] ?? null), ':why_asset_id' => AssetResolver::id($data['why_asset_id'] ?? null), ':sort_order' => max(0, (int)($data['sort_order'] ?? 0)), ':is_active' => !empty($data['is_active']) ? 1 : 0];
            if ($id === null) {
                $stmt = $pdo->prepare('INSERT INTO home_why_tabs (legacy_id,tab_name,title,why_asset_id,sort_order,is_active) VALUES (:legacy_id,:tab_name,:title,:why_asset_id,:sort_order,:is_active)');
                $stmt->execute($params);
                $id = (int)$pdo->lastInsertId();
            } else {
                $params[':id'] = $id;
                $stmt = $pdo->prepare('UPDATE home_why_tabs SET legacy_id=:legacy_id,tab_name=:tab_name,title=:title,why_asset_id=:why_asset_id,sort_order=:sort_order,is_active=:is_active WHERE id=:id');
                $stmt->execute($params);
            }
            ...
            foreach ((array)($data['items'] ?? []) as $order => $text) {
                $text = trim((string)$text);
                if ($text === '') continue;
                $itemStmt->execute([':tab_id' => $id, ':item_text' => $text, ':sort_order' => $order]);
            }
            $pdo->commit();
            return $id;
        } catch (Throwable $e) {
    ...
    public static function saveStatsBackground(string $assetId): void
    {
